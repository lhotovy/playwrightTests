...
test.describe("Testing landing page", () => {
  ...
  test('Testing presence of elements on landing page', async ({ landingPage }) => {   
      const { getHomeLink, eshopLink, contactLink, goToLoginButton, logoButton,
  ...
  test('Testing link attributes on landing page', async ({ landingPage }) => {
      const { getHomeLink, eshopLink, contactLink, goToLoginButton, 
      ...
      await expect(goToLoginButton).toHaveAttribute("href", "/login");
